...
def _resolve_telco_csv_uri():
    storage_options = _s3_storage_options_from_env()
    if TELCO_CSV_URI_ENV:
        if TELCO_CSV_URI_ENV.startswith("s3://"):
            if _exists_s3(TELCO_CSV_URI_ENV, storage_options):
    ...
    for cand in DEFAULT_CSV_CANDIDATES:
        if cand.startswith("s3://") and _exists_s3(cand, storage_options):
...
def main():
    ...
    clf = SGDClassifier(loss="log_loss", penalty="l2", alpha=LR_ALPHA, learning_rate="optimal", random_state=RANDOM_STATE, fit_intercept=True, max_iter=1, warm_start=False, tol=None)
    ...
    with start_run_with_retry(exp_id, RUN_NAME) as run:
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            for Xb, yb in batch_iter(X_train, y_train, BATCH_SIZE, shuffle=True, seed=RANDOM_STATE + epoch * 1000):
        ...
        print(f"[PROMOTE] accuracy={final_acc_num:.5f}")
